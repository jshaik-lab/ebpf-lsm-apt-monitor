...
[[ -f "$OUT/real_data_results_gcp.json" ]] || { run "capture-real-traces" bash src/python/capture_real_traces.sh; run "eval-real" python3 src/python/evaluate_real_data.py; meta results/evaluations/real_data_results.json "$OUT/real_data_results_gcp.json"; }
[[ -f "$OUT/ltl_fpr_real_gcp.json" && "${LTL_FORCE:-0}" != "1" ]] \
  || run "ltl-fpr-real" python3 src/python/evaluate_ltl_real.py --out "$OUT/ltl_fpr_real_gcp.json"
...
if [[ -f "$DARPA" ]]; then
  ...
  [[ -f "$OUT/darpa_ablation_gcp.json" && "${ABLATION_FORCE:-0}" != "1" ]] \
    || run "darpa-ablation" python3 src/python/evaluate_darpa_ablation.py --dataset cadets --max-windows 100 --strip-annotations --hard-fraction 0.5 --darpa-path "$DARPA" --out "$OUT/darpa_ablation_gcp.json"
fi
