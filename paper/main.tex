...
We instrument a repeated \texttt{open("race\_link")} loop ($N{=}1000$) while a
pthread adversary swaps \texttt{race\_link} between \texttt{safe.txt} and
\texttt{shadow\_target}.  For each successful \texttt{open}, we record the
pre-resolution pathname (analogous to \texttt{sys\_enter\_openat} argument) and
the post-resolution identity via \texttt{readlink(/proc/self/fd/$N$)} (analogous
to \texttt{lsm/file\_open} \texttt{bpf\_d\_path}).  Results are produced only on
the GCP VM (\texttt{toctou\_race\_gcp.json} in \texttt{run\_gcp\_eval\_chain.sh});
Mac and Docker runs are blocked by platform guards.
...
\textbf{(L8) TOCTOU eBPF confirmation.} The userspace open-vs-\texttt{readlink}
measurement runs on GCP only; an optional eBPF harness
(\texttt{scripts/run\_toctou\_benchmark.sh} with root +
\texttt{CONFIG\_BPF\_LSM}) attaches production \texttt{tp\_openat} and
\texttt{lsm\_file\_open} probes for kernel-level confirmation.
...
\textbf{see \texttt{red\_team\_results\_gcp.json}} \\
\bottomrule
\end{tabular}
\vspace{2pt}
