...
F1\,$=$\,0.603 (LLM-only: 0.597) and TI-aided F1\,$=$\,0.915; on 105 native-Linux
strace traces, \ipg{} reduces tokens by 74.0\% at window size~20 (92.7\% full trace)
...
We instrument a repeated \texttt{open("race\_link")} loop ($N{=}10{,}000$ open
attempts; victim and attacker pinned to CPU~0) while a pthread adversary swaps
...
The GCP artifact \texttt{toctou\_race\_gcp.json} reports
\texttt{userspace\_proxy.tracepoint\_miss\_rate}: among successful opens where
post-resolution hits \texttt{shadow\_target}, the fraction whose pre-resolution
pathname still reads \texttt{race\_link}---i.e., races invisible to a
tracepoint-only HIDS.
...
When root and \texttt{CONFIG\_BPF\_LSM} are available,
\texttt{run\_toctou\_benchmark.sh} also attaches kernel
\texttt{sys\_enter\_openat} vs.\ \texttt{lsm/file\_open} probes
(\texttt{ebpf\_lsm} field in the same artifact).
Low open yield ($\ll N$ successful opens) remains a reviewer-visible weakness;
reviewers may request tighter pinning or larger~$N$.
...
\noindent\textbf{False-positive breakdown ($\mathrm{FPR}{=}0.291$).}
Confusion-matrix analysis (\texttt{real\_data\_fpr\_breakdown\_gcp.json}) shows
16/55 benign traces flagged \textsc{Malicious}.  \textbf{13/16} originate from
...
the LLM maps to staged kill-chains at low confidence ($\approx$0.42).  The
remaining FPs are singletons (\texttt{benign\_cat\_os\_release},
\texttt{benign\_find\_libs}, \texttt{benign\_which\_bash}).  This failure mode
...
falls from 0.291 at $\theta_{\mathrm{low}}{=}0.8$ to 0.273 at the production
value 1.2 (4 traces gate-skipped) and 0.236 at 1.5/2.0, with TPR stable at
0.714 until $\theta_{\mathrm{low}}{=}2.0$ (0.571).  The production threshold
...
graph-first routing improves precision (0.957 vs.\ 0.852) and reduces FPR
(0.02 vs.\ 0.08) relative to LLM-only at comparable recall; 24/25
...
\textbf{(L7) EGTE not evaluated.} Evidence-Gated Tier-Calibrated Enforcement
(\texttt{sentinel/egte.py}) is implemented but disabled by default; its
calibration corpus is synthetic-only and is excluded from this evaluation.
\textbf{(L8) TOCTOU measurement scope.} The userspace open-vs-\texttt{readlink}
layer is a deliberate proxy for the tracepoint/LSM pathname delta; it is not
bitwise identical to production \texttt{lsm/file\_open} + \texttt{bpf\_d\_path}.
The same harness optionally attaches kernel eBPF probes when run with root on
GCP (\texttt{ebpf\_lsm} in \texttt{toctou\_race\_gcp.json}).  Neither layer
proves live blocking---only that telemetry placement matters; pair with EV-11
(Table~\ref{tab:redteam}) for end-to-end detection.
...
\texttt{llama3.1:8b} via Ollama on the GCP VM
(\texttt{evaluate\_red\_team.py}; artifact \texttt{red\_team\_results\_gcp.json}).
Per-layer columns indicate which defense \emph{would} catch each strategy in isolation.
Detection requires \emph{any} layer to fire: hard-trigger resources
(/etc/shadow, .ssh, /proc/self/mem), parent-PID propagation, LLM classification, LTL Symbolic Guardian
(RuntimeMonitor + Büchi), or PCABP static violation.
Aggregate detection rate and per-scenario LLM outcomes are read from the GCP
artifact after \texttt{run\_gcp\_eval\_chain.sh} (mock classifiers disabled).
...
\end{tabular}
\vspace{2pt}
{\scriptsize EV-10 through EV-15 are new scenarios requiring PCABP, LSM post-resolution, or LTL temporal logic to detect; they would evade a syscall-type-only or entropy-only detector. EV-11 models SENTINEL's TOCTOU architectural advantage over tracepoint-based detectors; the symlink-race micro-benchmark (\S\ref{sec:eval_lsm}) quantifies the tracepoint miss rate, while EV-11 confirms end-to-end hard-trigger + LSM detection. EV-14 evaded (DNS rebinding).}
\end{table}
...
\texttt{/tmp/safe} at tracepoint (\texttt{sys\_enter\_openat}), then a concurrent
...
the hard-trigger resource path (ISOLATE tier, conf\,=\,0.93 on GCP).
The micro-benchmark in \S\ref{sec:eval_lsm} measures how often that gap appears
under controlled racing; EV-11 confirms that \sentinel{} \emph{detects and enforces}
on the resolved path in the full pipeline---telemetry placement and end-to-end
blocking are separate claims.
