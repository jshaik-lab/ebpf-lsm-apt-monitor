...
On the GCP VM (\texttt{toctou\_race\_gcp.json}): 3{,}635 successful opens
(7.3\% yield) with userspace \texttt{tracepoint\_miss\_rate}\,$=$\,49.9\% among
...
The harness optionally attaches kernel eBPF probes when \texttt{bpf} appears in
\texttt{/sys/kernel/security/lsm} (not true on our GCP Ubuntu image despite
\texttt{CONFIG\_BPF\_LSM=y}).  Neither layer proves live blocking---only that
telemetry placement matters; pair with EV-11 (Table~\ref{tab:redteam}) for
end-to-end detection and enforcement.
