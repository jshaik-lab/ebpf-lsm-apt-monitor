...
this reduces token consumption by 57.5\% at $n{=}20$ events (measured with
tiktoken cl100k\_base on 15 GCP strace files; see \S\ref{sec:eval_setup}) while preserving the causal
...
On seven benign strace traces from \texttt{data/input/real\_traces} present on
the GCP VM at eval time (\texttt{cat}, \texttt{find}, \texttt{ls},
\texttt{python}, \texttt{which} workloads), the Symbolic Guardian raises
\textbf{zero violations} across 13 sliding 20-event windows:
FPR\,$=$\,0.000, bootstrap 95\% CI\,$=$\,[0.000, 0.000]. JSON:
\texttt{ltl\_fpr\_real\_gcp.json}. The broader 105-trace corpus
(\S\ref{sec:eval_real}) is evaluated separately for LLM classification; LTL
FPR on all 55 benign traces is deferred until the full corpus is staged on the
eval host.
...
\midrule
Credential dump  & T1003        & MALICIOUS & 0.98 & ISOLATE    & 43.1\,s \\
Privilege esc.   & T1068        & MALICIOUS & 0.93 & ISOLATE    & 6.5\,s \\
Process inject   & T1055        & MALICIOUS & 0.98 & ISOLATE    & 7.6\,s \\
...
Nginx server     & ---          & BENIGN    & 0.42 & LOG\_ONLY  & 6.0\,s \\
...
latency p50\,$=$\,7.4\,s (max\,$=$\,43.1\,s on the first-scenario Ollama cold start;
subsequent scenarios reuse the KV cache via \texttt{OLLAMA\_KEEP\_ALIVE=24h}).
...
Data Exfiltration (T1041) scores $c=0.95$ because its IPG contains only
...
IPG build + serialize         & 0.195\,ms   & 0.328\,ms     \\
Mock classifier               & 0.014\,ms  & 0.024\,ms    \\
CWAE enforce (dry-run)        & 1.023\,ms   & 1.349\,ms     \\
Detector throughput           & \multicolumn{2}{c}{27{,}454 events/s} \\
\midrule
...
the LTL FPR subset (\S\ref{sec:ltl}, 0/13 benign windows on seven traces in
...
gate-skip) yields TPR\,$=$\,0.714 and FPR\,$=$\,0.291 (accuracy\,$=$\,0.712;
bootstrap 95\% CI [0.625, 0.798]; $n{=}104$ evaluated, 1 skipped) in
...
\textbf{0.273} with unchanged TPR\,$=$\,0.714 (Acc\,$=$\,0.721); we report the
gated figure as the operational headline.
\noindent\textbf{False-positive breakdown ($\mathrm{FPR}{=}0.273$ at $\theta_{\mathrm{low}}{=}1.2$).}
Confusion-matrix analysis (\texttt{real\_data\_fpr\_breakdown\_gcp.json}) on the
ungated LLM run ($\mathrm{FPR}{=}0.291$) shows
...
8B model (F1\,$=$\,0.603). The Graph-only baseline (\texttt{graph\_only}) uses
...
\textbf{1 of 100} windows---achieving F1\,$=$\,0.603 with 75\,ms mean latency.
...
\midrule
LLM-only (\texttt{llm\_only})   & 0.603 & 0.440 & 0.020 & 100 & 7484\,ms \\
Graph-only (\texttt{graph\_only}) & 0.611 & 0.440 & 0.000 & 0   & $<$1\,ms \\
Hybrid (\texttt{hybrid})         & 0.603 & 0.440 & 0.020 & 1   & 75\,ms \\
Hybrid + LTL (\texttt{hybrid\_ltl}) & 0.611 & 0.440 & 0.000 & 1   & 75\,ms \\
Full (\texttt{full})             & 0.611 & 0.440 & 0.000 & 1   & 75\,ms \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
($\approx$7.5\,s each); hybrid invokes it once ($\approx$75\,ms amortized).
...
indistinguishable from a legitimate nginx worker at the syscall layer.
In the GCP Ollama run, both layers fire: the LLM classifies
\textsc{Malicious} (conf\,=\,0.83, QUARANTINE tier) and PCABP scores the
out-of-binary instruction pointer at 1.0 (heap $\gg$ nginx .text).
PCABP's value here is \emph{deterministic call-site proof} independent of
LLM judgment---critical when a smaller or miscalibrated model returns
\textsc{Benign} on syscall-identical mimicry (\S\ref{sec:pcabp}).
...
draft-tier T1210/T1041 false-negative gap;
...
(TPR\,=\,0.714, FPR\,=\,0.273 at production $\theta_{\mathrm{low}}{=}1.2$,
Acc\,=\,0.721; ungated LLM FPR\,=\,0.291), \sentinel{}
